...
export function get(provider) {
  const row = one('SELECT * FROM integrations WHERE provider = ?', provider);
  if (!row) return null;
  return {
    ...row,
...
export function getSecrets(provider) {
  const row = one('SELECT secret_json FROM integrations WHERE provider = ?', provider);
...
export function upsert(provider, { config, secrets, is_enabled, status, last_error } = {}) {
  ...
  run(
    `INSERT INTO integrations (provider, config_json, secret_json, is_enabled, status, last_error, last_checked_at)
     VALUES (@provider, @config_json, @secret_json, @is_enabled, @status, @last_error, datetime('now'))
...
export const clearSecrets = (provider) =>
  run(`UPDATE integrations SET secret_json = NULL, is_enabled = 0, status = 'not_configured',
       updated_at = datetime('now') WHERE provider = ?`, provider).changes;

/** Safe shape for the admin UI: shows whether a secret exists, never its value. */
export function listPublic() {
  return all('SELECT provider, is_enabled, config_json, status, last_checked_at, last_error, secret_json FROM integrations')
    .map(r => ({
      provider: r.provider,
