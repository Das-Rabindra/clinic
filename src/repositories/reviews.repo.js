...
export const findById = (id) => one('SELECT * FROM reviews WHERE id = ?', id);

/** Upsert by the provider's stable review id - re-syncs update, never duplicate. */
export function upsertByExternalId(r) {
  run(
    `INSERT INTO reviews (source, external_id, author_name, author_photo_url, rating, text,
     ...
     VALUES (@source, @external_id, @author_name, @author_photo_url, @rating, @text,
       @review_url, @reviewed_at, @reply_text, @replied_at, @raw_json, datetime('now'))
     ON CONFLICT(external_id) DO UPDATE SET
       ...
       raw_json = excluded.raw_json, synced_at = datetime('now'), updated_at = datetime('now')`,
    {
...
export const listPublic = (limit = 12) =>
  all(`SELECT id, author_name, author_photo_url, rating, text, review_url, reviewed_at, is_featured
       FROM reviews WHERE is_visible = 1
       ORDER BY is_featured DESC, reviewed_at DESC, id DESC LIMIT ?`, limit);

export const listAdmin = () =>
  all('SELECT * FROM reviews ORDER BY reviewed_at DESC, id DESC');
...
export const aggregate = () =>
  one(`SELECT COUNT(*) AS count, ROUND(AVG(rating), 1) AS average
       FROM reviews WHERE is_visible = 1`);

export const setVisible = (id, visible) =>
  run(`UPDATE reviews SET is_visible = ?, updated_at = datetime('now') WHERE id = ?`,
    visible ? 1 : 0, id).changes;

export const setFeatured = (id, featured) =>
  run(`UPDATE reviews SET is_featured = ?, updated_at = datetime('now') WHERE id = ?`,
    featured ? 1 : 0, id).changes;

export const recent = (limit = 5) =>
  all('SELECT * FROM reviews ORDER BY synced_at DESC, id DESC LIMIT ?', limit);

export const count = () => one('SELECT COUNT(*) AS c FROM reviews').c;

/* Sync state (singleton row) */
export const syncState = () => one('SELECT * FROM review_sync_state WHERE id = 1');
  ...
  run(
    `INSERT INTO review_sync_state (id, connected, account_name, location_name, location_title,
     ...
     VALUES (1, @connected, @account_name, @location_name, @location_title,
       @rating_avg, @rating_count, @last_sync_at, @last_error, @last_error_at, datetime('now'))
