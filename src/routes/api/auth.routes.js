...
router.post('/login', loginLimiter, validate(loginSchema), (req, res) => {
  try {
    const { token, csrfToken, user } = auth.login(req.body, {
      ip: req.ip, userAgent: String(req.get('user-agent') || '').slice(0, 300),
    });
    res.cookie(config.session.cookieName, token, auth.sessionCookieOptions());
...
router.post('/logout', (req, res) => {
  auth.logout(req.cookies?.[config.session.cookieName], ctxFrom(req));
...
router.post('/change-password', requireAuth, requireCsrf,
  ...
  (req, res) => {
    try {
      auth.changePassword(req.user.id, {
...
router.post('/forgot-password', resetLimiter, validate(z.object({ email: zEmail })),
  asyncHandler(async (req, res) => {
    const result = auth.requestPasswordReset(req.body.email, { ip: req.ip });
    if (result.issued) {
...
router.post('/reset-password',
  ...
  (req, res) => {
    try {
      auth.resetPassword({ token: req.body.token, newPassword: req.body.new_password }, { ip: req.ip });
...
router.get('/sessions', requireAuth, (req, res) => {
  res.json(sessionsRepo.listForUser(req.user.id).map(s => ({
    ...s, current: s.id === req.session.id,
  })));
});

router.post('/sessions/revoke-others', requireAuth, requireCsrf, (req, res) => {
  const all = sessionsRepo.listForUser(req.user.id);
  let n = 0;
  for (const s of all) if (s.id !== req.session.id) n += sessionsRepo.revoke(s.id);
