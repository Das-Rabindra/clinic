...
| Menu panel nested inside a `backdrop-filter` header | A filter makes an element the containing block for its `position:fixed` descendants, so the panel's `inset` resolved against the 70px header rather than the viewport: it opened as a **40px sliver with the page showing through**. Hidden behind the bug above for as long as the menu never opened at all, and invisible to a check that only asks whether `display` changed. | Panel moved out to be a sibling of `<header>`; test asserts it appears after `</header>` on every page |
| Header brand line pushed the page sideways | Putting the full address in the brand sub-line replaced an 18-character name with a 32-character one, and the line is `nowrap` — 53px of horizontal scroll on a 414px phone. Narrow phones passed because below 400px the header CTA is hidden and the shorter row fit, so the **wider** viewport was the broken one. | Short "Bikrampur · Talcher" in the header, full address in the body, and `min-width:0` on the brand block so nothing put there can widen the header again |
